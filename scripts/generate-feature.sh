...
# Usage: ./scripts/generate-feature.sh appointments Appointment

set -euo pipefail

if [ $# -lt 2 ]; then
  echo "Usage: $0 <kebab-name> <PascalName>"
  echo "Example: $0 appointments Appointment"
  exit 1
fi
...
render() {
  sed -e "s/__feature__/$KEBAB/g" -e "s/__Feature__/$PASCAL/g" "$1"
}
